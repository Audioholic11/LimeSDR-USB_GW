...
process(rclk, reset_n,inst_chirp_sync_en)
begin
   if reset_n = '0' then 
      pct_size_only_data <= (others=>'1');
   elsif (rclk'event AND rclk='1') then 
      if (inst_chirp_sync_en='1') then
...
p2d_wr_fsm_inst0 : entity work.p2d_wr_fsm
   ...
   port map(
      ...
      pct_buff_rdy      => instx_wrempty,
		chirp_sync_en		=> inst_chirp_sync_en
      );
...
begin
   if reset_n = '0' then 
      ...
      for i in 0 to n_buff-1 loop
         if unsigned(instx_rdusedw(i)) > half_pct_size_only_data-1 then 
